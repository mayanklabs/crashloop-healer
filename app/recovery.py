...
from app.metrics import container_restarts_total, container_failures_total, recovery_duration_seconds
...
def _recreate_container(container_name: str, image: str, inspect: dict, run_args: dict) -> str:
...
def recover(container_name: str) -> dict:
    ...
    reason = f"OOMKilled: {oom_killed}, ExitCode: {exit_code}, RestartCount: {restart_count}"
    ...
    if restart_count < RESTART_THRESHOLD:
        ...
        _recreate_container(container_name, image_before, inspect, run_args)
        action = "restart"
        image_after = image_before
        restart_counts[container_name] = restart_count + 1
    else:
        ...
        if healthy_image and healthy_image != image_before:
            ...
            action = "rollback"
            restart_counts[container_name] = 0
        else:
            backoff = min(2 ** restart_count, MAX_BACKOFF)
            time.sleep(backoff)
            container.remove(force=True)
    ...
    recovery_duration_seconds.labels(container_name=container_name, action=action).observe(duration_ms / 1000)
